...
class Aes
{
    // Jumlah kata 32-bit yang terdiri dari plaintext dan kolom yang terdiri dari matriks status dari AES cipher .
    private static $Nb = 4;
    // Jumlah kata 32-bit yang terdiri dari cipher key dalam AES cipher AES .
    private $Nk;
    // Jumlah putaran dalam AES chiper 
    private $Nr;
    ...
    // Jadwal kunci dalam sandi AES ini.
    private $w = array(); // Nb*(Nr+1) 32-bit words
    // Matriks keadaan dalam sandi AES ini dengan kolom Nb dan 4 baris
    private $s = array(array());
    // Mode operasi cipher blok
    private $mode;
    // Vektor inisialisasi yang digunakan dalam mode sandi lanjutan
    private $iv;
    ...
    */
    public function __construct($z, $mode = 'ECB', $iv = null)
    {
        ...
        $this->keyExpansion($z); // menempatkan kunci yang diperluas di w
    }
    ...
    **/
    ...
    {
        $t = '';// Blok 16-byte untuk menampung input sementara dari cipher
        $y = ''; // returned cipher text;
        $y_block = $this->iv; // Blok 16-byte untuk menampung output sementara dari cipher
        $xsize = strlen($x);

        switch ($this->mode) {
            case 'ECB':
                // masukkan blok 16-byte ke dalam t, ekkripsi dan tambahkan ke hasilnya
            ...
            case 'CBC':
                ...
                // XOR blok plaintext ini dengan vektor inisialisasi
            ...
            case 'CFB':
                for ($i = 0; $i < $xsize; $i += 16) {
                    // Enkripsi output vektor/cipher inisialisasi lalu XOR dengan plaintext
                    $y_block = $this->encryptBlock($y_block);

                    for ($j = 0; $j < 16; $j++) {
                        // XOR output cipher dengan plaintext
    ...
    **/
    public function decrypt($y)
    {
        ...
        switch ($this->mode) {
            ...
            case 'CBC':
                for ($i = 0; $i < $ysize; $i += 16) {
                    ...
                    // XOR blok cipher iv/sebelumnya dengan blok cipher yang didekripsi ini
                    for ($j = 0; $j < 16; $j++) {
            ...
            case 'CFB':
                for ($i = 0; $i < $ysize; $i += 16) {
                    // Enkripsi vektor inisialisasi/output cipher kemudian XOR dengan ciphertext
                    $x_block = $this->encryptBlock($y_block);

                    for ($j = 0; $j < 16; $j++) {
                        // XOR output cipher dengan ciphertext.
        ...
        return rtrim($x, chr(0)); // Hapus semua residu buffer saat kembali.
    }
    ...
    **/
    public function encryptBlock($x)
    {
        ...
        // tempatkan input x ke dalam matriks keadaan awal dalam urutan kolom
        for ($i = 0; $i <4*self::$Nb; $i++) {
            // kita ingin pembagian bilangan bulat untuk indeks kedua
       ...
       // tambahkan kunci bulat
        ...
        // tempatkan matriks keadaan s ke dalam y dalam urutan kolom
        for ($i = 0; $i < 4*self::$Nb; $i++) {
    ...
    **/
    public function decryptBlock($y)
    {
        ...
        // tempatkan input y ke dalam matriks keadaan awal dalam urutan kolom
        for ($i = 0; $i < 4*self::$Nb; $i++) {
       ...
       // tempatkan matriks keadaan s ke dalam x dalam urutan kolom
        for ($i = 0; $i < 4*self::$Nb; $i++) {
            // Digunakan untuk menghapus karakter null yang terisi.
    ...
    **/
    private function keyExpansion($z)
    {
        ...
        $temp = 0; // kata 32-bit sementara

        // kata Nk pertama dari w adalah kunci sandi z
        for ($i = 0; $i < $this->Nk; $i++) {
            $this->w[$i] = 0;
            // isi seluruh kata kunci yang diperluas w
            // dengan mendorong 4 byte ke dalam kata w[i]
    ...
    **/
    private function addRoundKey($round)
    {
        ...
        for ($i = 0; $i < 4; $i++) {
            for ($j = 0; $j < self::$Nb; $j++) {
                // tempatkan byte ke-i dari kata ke-j dari kunci yang diperluas w ke temp
                $temp = $this->w[$round*self::$Nb+$j] >> (3-$i)*8;
                // Cast temp dari kata 32-bit menjadi byte 8-bit.
                $temp %= 256;
                // Tidak dapat melakukan shift tanpa tanda, jadi kita perlu membuat suhu ini positif
                $temp = ($temp < 0 ? (256 + $temp) : $temp);

                $this->s[$i][$j] ^= $temp; // xor temp dengan byte di lokasi (i,j) status
            }
        }
    }
    ...
    **/
    ...
    {
        $temp = $w >> 24; // masukkan 8-bit pertama ke dalam temp
        $w <<= 8; // beri ruang bagi temp untuk mengisi ujung bawah kata
    ...
    **/
    ...
    {
        $temp = 0;
        // loop melalui 4 byte dari sebuah kata
        for ($i = 0; $i < 4; $i++) {
            $temp = $w >> 24; // masukkan 8-bit pertama ke dalam temp
            // Tidak dapat melakukan shift tanpa tanda, jadi kita perlu membuat suhu ini positif
            $temp += $temp < 0 ? 256 : 0;
            $w <<= 8; // beri ruang untuk byte yang diganti di w;
            self::make32BitWord($w);
            $w += self::$sBox[$temp]; // tambahkan kembali byte yang diganti
        }
    ...
    **/
    ...
    {
        // Kurangi kata 64-bit ini menjadi 32-bit pada mesin 64-bit
